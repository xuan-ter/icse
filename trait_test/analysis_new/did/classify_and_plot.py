...
def _resolve_vertical_overlaps(labels, y_min, y_max, min_gap=8, max_push=16):
    adjusted = []
    labels_sorted = sorted(labels, key=lambda x: x[1], reverse=True)
    last_y = None
...
def plot_top_interactions_forest_pil(rows, output_png_path, top_n=50):
    ...
    font_sub = _load_font(int(round(40 * scale)), bold=False)
    ...
    try:
        tb_sub = draw.textbbox((0, 0), subtitle_txt, font=font_sub)
    ...
    draw.text(((width - tw_sub) // 2, int(round(140 * title_scale))), subtitle_txt, fill=(70, 70, 70), font=font_sub)
    ...
    draw.rectangle([x0, y0, x1, y1], fill=(248, 248, 248, 255), outline=None)
    ...
    for xt in x_ticks:
        x = x_to_px(xt)
        draw.line([(x, y0), (x, y1)], fill=(230, 230, 230, 255), width=int(round(2 * scale)))
...
def plot_delta_distributions_three_types(neg_deltas, pos_deltas, ind_deltas, output_png_path, bins=50, x_max=None, text_scale=1.0, height_scale=1.0, legend_scale=1.0):
    ...
    for i, (label, values, color) in enumerate(panels):
        ...
        draw.rectangle([left, y0, left + panel_w, y1], fill=(248, 248, 248, 255), outline=None)
        ...
        for x_txt, y_txt, tw, th, txt in resolved_labels:
            pad_x, pad_y = 4, 2
            draw.rectangle([x_txt - pad_x, y_txt - pad_y, x_txt + tw + pad_y, y_txt + th + pad_y], fill=(255, 255, 255, 235))
        ...
        if i == 0:
            draw.text((left - int(round(130 * non_title_scale)), y0 - int(round(60 * non_title_scale))), "Count", fill=(0, 0, 0), font=font_tick)
...
def plot_kde_overlay_three_types(neg_deltas, pos_deltas, ind_deltas, output_png_path, x_max=None, text_scale=1.0):
    ...
    for xt in x_ticks:
        x = x_to_px(xt)
        draw.line([(x, y0), (x, y1)], fill=(230, 230, 230, 255), width=2)
...
def plot_kde_panels_three_types(neg_deltas, pos_deltas, ind_deltas, output_png_path, x_max=None):
    ...
    draw.text((90, 92), subtitle, fill=(70, 70, 70), font=font_sub)
...
def main():
    ...
    has_type_col = HAVE_MPL and ("interaction_type" in df.columns)

    # Classification Logic
    if HAVE_MPL:
        if has_type_col:
            neg_df = df[df["interaction_type"] == "negative_interaction"].copy()
        else:
    ...
    if HAVE_MPL:
        if has_type_col:
            pos_df = df[df["interaction_type"] == "positive_interaction"].copy()
        else:
    ...
    if HAVE_MPL:
        if has_type_col:
            ind_df = df[df["interaction_type"] == "independent"].copy()
        else:
    ...
    ind_df.sort_values('delta', key=abs, ascending=False).to_csv(ind_csv, index=False)
    ...
    print(f"Saved distribution plot to {dist_out}")
    print(f"Saved distribution plot to {os.path.splitext(dist_out)[0] + '.pdf'}")
    ...
    plot_top_interactions_forest_pil(df.to_dict('records'), os.path.join(BASE_DIR, "top_interactions_forest.png"), top_n=20)
    print(f"Saved top-20 forest plot to {os.path.join(BASE_DIR, 'top_interactions_forest.png')}")
    print(f"Saved top-20 forest plot to {os.path.join(BASE_DIR, 'top_interactions_forest.pdf')}")
