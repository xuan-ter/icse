...
DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "results", "20260615_153358"))
...
def normalize_pass_names(df):
    ...
    # 使用 Case 列作为 Variant（如果存在）
    if "Case" in df.columns:
        df["Variant"] = df["Case"].astype(str).str.strip()
    elif "Variant" in df.columns:
        df["Variant"] = df["Variant"].astype(str).str.strip()
    else:
        # 如果都没有，创建一个默认的 Variant
        df["Variant"] = "default"
...
def analyze_interactions(raw_df, metric="TotalRuntime(s)", n_bootstrap=BOOTSTRAP_SAMPLES, seed=BOOTSTRAP_SEED):
    ...
    expected_pairs = len(mir_passes) * len(llvm_passes)
    print(f"Expected MIR x LLVM combinations: {expected_pairs}")
...
def plot_forest(results_df):
    ...
    plt.title("Top Interactions from Raw-Run Variant-Aware Bootstrap DiD")
    plt.tight_layout()
...
def main():
    ...
    results_path = os.path.join(OUTPUT_DIR, "interaction_results.csv")
