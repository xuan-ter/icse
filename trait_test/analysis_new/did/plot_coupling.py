...
def plot_filtered_heatmap(data, output_path):
    """
    Plot only significant interactions and mask the rest.
    """
    if HAVE_PANDAS and HAVE_SEABORN:
        ...
        plt.close()
        print(f"Saved filtered heatmap to {pdf_path}")
        print(f"Saved filtered heatmap to {png_path}")
        return
    ...
    llvm_order = {}
    for row in rows:
        if not fbool(row.get("significant")):
            continue
        mir = str(row.get("mir_pass", "")).strip()
        llvm = str(row.get("llvm_pass", "")).strip()
        if mir and mir not in mir_order:
            mir_order[mir] = len(mir_order)
        if llvm and llvm not in llvm_order:
            llvm_order[llvm] = len(llvm_order)
    ...
    for row in rows:
        ...
        if mir not in mir_order or llvm not in llvm_order:
            continue
        delta = ffloat(row.get("delta"))
        if delta != delta:
            continue
        mat[mir_order[mir], llvm_order[llvm]] = delta
...
def plot_clustermap(df, output_path):
    """
    Cluster similar interaction patterns using significant interactions.
    """
    sig_df = df[df["significant"] == True]
    if sig_df.empty:
        print("No significant interactions found for clustermap.")
        return
    ...
    # Seaborn/scipy cannot build a distance matrix from a single row/column.
    if data_for_clustering.shape[0] < 2 or data_for_clustering.shape[1] < 2:
        print("Not enough significant interactions for clustermap.")
        return
    ...
    pdf_path, png_path = save_figure_variants(g.fig, output_path)
    plt.close()
    print(f"Saved clustermap to {pdf_path}")
    print(f"Saved clustermap to {png_path}")


def plot_bipartite_network(df, output_path, top_n=50):
    """
    Plot a bipartite graph for the strongest significant interactions.
    """
    sig_df = df[df["significant"] == True].copy()
    if sig_df.empty:
        print("No significant interactions found for network.")
        return
    ...
    plt.title(
        f"Top {top_n} Strongest Cross-Layer Couplings (Bipartite Graph)\n"
        "Blue=Negative (Redundancy), Red=Positive (Interference)"
    )
    plt.axis("off")
    ...
    pdf_path, png_path = save_figure_variants(fig, output_path)
    plt.close()
    print(f"Saved bipartite network to {pdf_path}")
    print(f"Saved bipartite network to {png_path}")
