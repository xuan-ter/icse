...
输入：JSON 配置文件（包含随机生成的 Pass 开关组合）
输出：CSV 格式的实验结果（包括二进制大小、运行时、编译时等指标）
描述：
...
DEFAULT_SEARCH_SPACE = os.path.join(os.path.dirname(PROJECT_ROOT), "table", "table_json", "mir_llvm_double_disable_matrix.json")
...
def measure_combination(combo, runs=1, skip_clean=False):
    ...
    # Loop runs: (optional) Clean -> Build -> Measure Size -> Run
