...
PROJECT_ROOT = "/root/MIR_LLVM/branch_cfg_bench"
BINARY_PATH = os.path.join(PROJECT_ROOT, "target", "release", "cfg_test")
DEFAULT_JSON_PATH = "/root/MIR_LLVM/table/table_json/combined_experiment_matrix.json"
DEFAULT_RESULTS_ROOT = "/root/MIR_LLVM/branch_cfg_bench/results"
...
def rustflags_with_evidence(rustflags, mir_dump_dir):
    flags = rustflags
    if "--emit=llvm-ir,link" in flags:
        flags = flags.replace("--emit=llvm-ir,link", "--emit=llvm-ir,asm,link")
    else:
        flags = f"{flags} --emit=llvm-ir,asm,link"
    return (
        f"{flags} -Z dump-mir=workload_"
...
def newest_llvm_ir_path():
    cand = glob.glob(os.path.join(PROJECT_ROOT, "target", "release", "deps", "cfg_test-*.ll"))
    if not cand:
        return ""
...
def newest_asm_path():
    patterns = [
        os.path.join(PROJECT_ROOT, "target", "release", "deps", "cfg_test-*.s"),
        os.path.join(PROJECT_ROOT, "target", "release", "cfg_test.s"),
    ]
    cand = []
...
def collect_mir_files(root_dir):
...
def persist_evidence_artifacts(out_dir, config_name, run_id, ll_path, asm_path, mir_dump_dir):
    run_dir = os.path.join(out_dir, "evidence", safe_dir_name(config_name), f"run_{run_id}")
    os.makedirs(run_dir, exist_ok=True)

    kept_ll = ""
    kept_asm = ""
    if ll_path:
        kept_ll = copy_if_exists(ll_path, os.path.join(run_dir, "llvm_ir", os.path.basename(ll_path)))
    if asm_path:
        kept_asm = copy_if_exists(asm_path, os.path.join(run_dir, "asm", os.path.basename(asm_path)))

    kept_mir_files = collect_mir_files(mir_dump_dir)
        ...
        json.dump(
            {
                ...
                "run_id": run_id,
                "llvm_ir_path": kept_ll,
                "asm_path": kept_asm,
                "mir_file_count": len(kept_mir_files),
            },
            f,
            ensure_ascii=True,
            indent=2,
...
def run_binary(env, variant, n, iters, seed):
    args = [
        BINARY_PATH,
        "--variant",
        variant.lower(),
...
def run_binary_repeated(env, variant, n, iters, seed, repeats):
...
def calibrate_repeats(env_base, baseline_combo, variants, n, iters, seed, target_seconds, max_repeats, skip_clean):
    ...
    for v in variants:
        t, _ = run_binary(env2, v, n, iters, seed)
        if t is None or t <= 0:
...
def main():
    ...
    ap.add_argument("--target-seconds", type=float, default=10)
    ap.add_argument("--max-repeats", type=int, default=1000)
    ...
    with open(results_csv, "w", encoding="utf-8", newline="") as f:
        ...
        for combo in combos:
            ...
            base_rustflags = compose_rustflags_from_combo(combo)
            rustflags = base_rustflags
            mir_dump_dir = ""
            if args.keep_evidence:
                mir_dump_dir = os.path.join(
                    out_dir,
                    "evidence",
                    safe_dir_name(name),
                    "build",
                ...
                rustflags = rustflags_with_evidence(base_rustflags, mir_dump_dir)
            ...
            asm_path = newest_asm_path() if args.keep_evidence else ""
            if args.keep_evidence:
                ll_path = persist_evidence_artifacts(out_dir, name, 1, ll_path, asm_path, mir_dump_dir)
            ...
            for variant in variants:
                ...
                for _ in range(max(args.warmup, 0)):
                    warmup_wall, _ = run_binary_repeated(env2, variant, args.n, args.iters, args.seed, repeats)
                    if warmup_wall is None:
                        warmup_failed = True
                        break
                ...
                for run_id in range(1, max(args.runs, 1) + 1):
                    ...
                    wall, _ = run_binary_repeated(env2, variant, args.n, args.iters, args.seed, repeats)
                    if wall is None:
                        w.writerow(
                            {
                                "ConfigName": name,
