...
DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "results", "20260608_201933"))
MAPPING_DIR = os.path.abspath(os.path.join(BASE_DIR, ".."))
OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))
DID_RESULTS_CSV = os.path.abspath(os.path.join(BASE_DIR, "..", "did", "interaction_results.csv"))
...
def load_and_preprocess_data():
    """
    Loads summary median CSV data and converts it into a feature matrix X and target vector y
    suitable for Lasso regression.
    
    X: Binary matrix where 1 means the Pass is DISABLED.
    y: Log-transformed median runtime.
    """
    print("Loading data...")
    # Load config to pass mapping
    mapping_path = os.path.join(MAPPING_DIR, "config_to_pass_mapping.csv")
    mapping_df = pd.read_csv(mapping_path)
    
    # Load summary medians
    summary_path = os.path.join(DATA_DIR, "summary_medians.csv")
    summary_df = pd.read_csv(summary_path)
    
    # Merge them to get MIR_Pass, LLVM_Pass, and median runtime
    full_df = pd.merge(summary_df, mapping_df, on='ConfigName', how='left')
    
    # Use wall_med as the runtime metric
    full_df['TotalRuntime(s)'] = full_df['wall_med']
    ...
    full_df = full_df[(full_df["MIR_Pass"] != "All") & (full_df["LLVM_Pass"] != "All")].copy()
...
def run_lasso_stability_selection(X, y, feature_names, n_bootstrap=100):
    """
    Runs Lasso on median data. Since we only have one sample per configuration,
    we'll use a single Lasso fit with cross-validation to find alpha.
    """
    print("Running Lasso on median data...")
    
    # Use LassoCV to find the best alpha with cross-validation
    print("Tuning hyperparameters with cross-validation...")
    model_cv = LassoCV(cv=min(5, X.shape[0]), random_state=42, n_jobs=-1, max_iter=10000)
    model_cv.fit(X, y)
    best_alpha = model_cv.alpha_
    print(f"Best alpha found: {best_alpha}")
    
    # Now fit a final Lasso model with the best alpha
    lasso = Lasso(alpha=best_alpha, max_iter=10000)
    lasso.fit(X, y)
    
    # Since we can't do bootstrap stability selection with only one sample per config,
    # we'll use the coefficients directly and set stability to 1 for non-zero coefficients
    coeffs = lasso.coef_
    stability_scores = np.array([1.0 if abs(c) > 1e-5 else 0.0 for c in coeffs])
    
    return stability_scores, coeffs

def save_coupling_graph(stability_scores, avg_coeffs, feature_names, threshold=0.0):
    """
    Extracts significant interactions and saves them.
    For median data, we use coefficient magnitude instead of stability scores.
    """
    print("Extracting significant interactions...")
    edges = []
    
    # First, find all interaction features with non-zero coefficients
    interaction_coeffs = []
    for i, (score, coeff, name) in enumerate(zip(stability_scores, avg_coeffs, feature_names)):
        # Check if it's an interaction feature (contains '|')
        if '|' in name and abs(coeff) > 1e-5:
            interaction_coeffs.append((i, abs(coeff), coeff, name))
    
    # Sort by coefficient magnitude and take top ones
    interaction_coeffs.sort(key=lambda x: x[1], reverse=True)
    top_n = min(100, len(interaction_coeffs))  # Take top 100 or all
    
    for i, abs_coeff, coeff, name in interaction_coeffs[:top_n]:
        parts = name.split('|')
        mir = parts[0]
        llvm = parts[1]
        ...
        edges.append({
            ...
            'Type': 'Interaction',
            'Weight': coeff,
            'Stability': 1.0  # All selected are stable in this single fit
        })
...
def build_proxy_edges_from_did(top_n=100):
    """
    Fall back to DiD results when Lasso collapses to all-zero interaction coefficients.
    """
    if not os.path.exists(DID_RESULTS_CSV):
        return pd.DataFrame(columns=['Source', 'Target', 'Type', 'Weight', 'Stability'])

    did_df = pd.read_csv(DID_RESULTS_CSV)
    if did_df.empty:
        return pd.DataFrame(columns=['Source', 'Target', 'Type', 'Weight', 'Stability'])

    if 'abs_delta' not in did_df.columns:
        did_df['abs_delta'] = did_df['delta'].abs()

    proxy_df = (
        did_df
        .sort_values(['significant', 'abs_delta'], ascending=[False, False])
        .head(top_n)
        .copy()
    )
    ...
    proxy_df['Type'] = 'Interaction'
    proxy_df['Stability'] = np.where(proxy_df['significant'].fillna(False), 1.0, 0.5)
    proxy_df = proxy_df[['Source', 'Target', 'Type', 'Weight', 'Stability']]

    output_path = os.path.join(OUTPUT_DIR, 'coupling_edges.csv')
    proxy_df.to_csv(output_path, index=False)
    print(f"Saved {len(proxy_df)} proxy edges from DiD results to {output_path}")
    return proxy_df
...
def plot_coupling_graph(df_edges, mir_passes, llvm_passes):
    ...
    plt.title("Cross-Level Coupling Graph (Lasso on Median Data)\nSquare=MIR, Circle=LLVM | Red=Conflict(+), Blue=Synergy(-)")
    plt.axis('off')
...
if __name__ == "__main__":
    if HAVE_LASSO_STACK:
        try:
            ...
            df_edges = save_coupling_graph(stability, coeffs, feature_names, threshold=0.0)
            if df_edges.empty:
                print("Lasso produced no interaction edges; falling back to DiD proxy edges...")
                df_edges = build_proxy_edges_from_did(top_n=100)
            plot_coupling_graph(df_edges, mir_passes, llvm_passes)
            out_png = os.path.join(OUTPUT_DIR, "lasso_coupling_matrix_top25x25.png")
            plot_coupling_matrix_from_edges_csv(
                os.path.join(OUTPUT_DIR, "coupling_edges.csv"),
                out_png,
                top_mir=25,
                top_llvm=25,
            )
            print("\n=== Top Recovered Interactions (by coefficient magnitude) ===")
            if not df_edges.empty:
                print(df_edges.sort_values('Weight', key=abs, ascending=False).head(10))
