...
DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "results", "run_20260602_123232"))
OUTPUT_DIR = BASE_DIR
...
def normalize_pass_names(df):
    """
    Normalize baseline spellings so all baseline rows become (None, None).
    """
...
def build_variant_samples_lookup(df, metric):
    grouped = (
        df[["Variant", "MIR_Pass", "LLVM_Pass", metric]]
        .dropna(subset=["Variant", "MIR_Pass", "LLVM_Pass", metric])
        .groupby(["Variant", "MIR_Pass", "LLVM_Pass"])[metric]
    )
    return {
        (variant, mir, llvm): values.to_numpy(dtype=float)
        for (variant, mir, llvm), values in grouped
    }
...
def bootstrap_interaction_by_variant(variant_samples, n_bootstrap, seed):
    """
    Bootstrap DiD inside each variant, then average variant-level deltas.
    """
    rng = np.random.default_rng(seed)
    boot_deltas = []
    point_deltas = []
    mean_logs = {"z00": [], "z10": [], "z01": [], "z11": []}

    for z00, z10, z01, z11 in variant_samples:
        ...
        boot_deltas.append((boot01 - boot11) - (boot00 - boot10))
        point_deltas.append(float((z01.mean() - z11.mean()) - (z00.mean() - z10.mean())))
        mean_logs["z00"].append(float(z00.mean()))
        mean_logs["z10"].append(float(z10.mean()))
        mean_logs["z01"].append(float(z01.mean()))
        mean_logs["z11"].append(float(z11.mean()))

    boot_delta = np.mean(np.vstack(boot_deltas), axis=0)
    ...
    return {
        ...
        "z00_mean": float(np.mean(mean_logs["z00"])),
        "z10_mean": float(np.mean(mean_logs["z10"])),
        "z01_mean": float(np.mean(mean_logs["z01"])),
        "z11_mean": float(np.mean(mean_logs["z11"])),
    }
...
def classify_effect_strength(results_df):
    """
    Classify interactions using bootstrap CI for significance and abs(delta)
    quantiles for effect tiers.
    """
    ...
    results_df["significant"] = ((results_df["ci_low"] > 0.0) | (results_df["ci_high"] < 0.0)) & (results_df["p_adj"] < 0.05)
    results_df["crosses_zero"] = ~results_df["significant"]
    results_df["interaction_type"] = "independent"
    results_df.loc[results_df["significant"] & (results_df["delta"] < 0), "interaction_type"] = "negative_interaction"
    results_df.loc[results_df["significant"] & (results_df["delta"] > 0), "interaction_type"] = "positive_interaction"
...
def analyze_interactions(raw_df, metric="TotalRuntime(s)", n_bootstrap=BOOTSTRAP_SAMPLES, seed=BOOTSTRAP_SEED):
    """
    Run variant-aware bootstrap DiD on raw repeated runs for all MIR x LLVM pairs.
    """
    samples_lookup = build_variant_samples_lookup(raw_df, metric)
    variants = sorted(raw_df["Variant"].unique())
    mir_passes = sorted({mir for _, mir, llvm in samples_lookup.keys() if mir != "None" and llvm == "None"})
    llvm_passes = sorted({llvm for _, mir, llvm in samples_lookup.keys() if mir == "None" and llvm != "None"})

    print(f"Found {len(variants)} variants, {len(mir_passes)} MIR passes, and {len(llvm_passes)} LLVM passes.")
    print(f"Expected MIR x LLVM combinations: {len(mir_passes) * len(llvm_passes)}")
    ...
    for mir_pass in mir_passes:
        for llvm_pass in llvm_passes:
            ...
            for variant in variants:
                y00 = samples_lookup.get((variant, "None", "None"))
                y10 = samples_lookup.get((variant, mir_pass, "None"))
                y01 = samples_lookup.get((variant, "None", llvm_pass))
                y11 = samples_lookup.get((variant, mir_pass, llvm_pass))
                ...
                z01 = np.log(y01)
                z11 = np.log(y11)
                variant_samples.append((z00, z10, z01, z11))
            ...
            boot = bootstrap_interaction_by_variant(
                variant_samples,
                n_bootstrap=n_bootstrap,
                seed=seed + len(results),
            )

            y00_geo = float(np.exp(boot["z00_mean"]))
            y10_geo = float(np.exp(boot["z10_mean"]))
            y01_geo = float(np.exp(boot["z01_mean"]))
            y11_geo = float(np.exp(boot["z11_mean"]))
            interaction_ratio = float(np.exp(boot["delta"]))
            interaction_pct = (interaction_ratio - 1.0) * 100.0
            mir_drop_pct = (float(np.exp(boot["z10_mean"] - boot["z00_mean"])) - 1.0) * 100.0
            llvm_drop_pct = (float(np.exp(boot["z01_mean"] - boot["z00_mean"])) - 1.0) * 100.0
            joint_drop_pct = (float(np.exp(boot["z11_mean"] - boot["z00_mean"])) - 1.0) * 100.0
            ...
            results.append(
                {
                    ...
                    "variants_used": ",".join(used_variants),
                }
            )
...
def plot_variant_heatmaps(raw_df, output_dir):
    """
    Save one heatmap per Variant using variant-local DiD estimates.
    """
    samples_lookup = build_variant_samples_lookup(raw_df, "TotalRuntime(s)")
    variants = sorted(raw_df["Variant"].unique())
    mir_passes = sorted({mir for _, mir, llvm in samples_lookup.keys() if mir != "None" and llvm == "None"})
    llvm_passes = sorted({llvm for _, mir, llvm in samples_lookup.keys() if mir == "None" and llvm != "None"})
    ...
    for variant in variants:
        ...
        for mir_pass in mir_passes:
            for llvm_pass in llvm_passes:
                ...
                z01 = np.log(y01)
                z11 = np.log(y11)
                delta = float((z01.mean() - z11.mean()) - (z00.mean() - z10.mean()))
                rows.append(
                    {
                        "mir_pass": mir_pass,
                        "llvm_pass": llvm_pass,
        ...
        pivot_delta = variant_df.pivot(index="mir_pass", columns="llvm_pass", values="delta")
        plt.figure(figsize=(12, 10))
        sns.heatmap(pivot_delta, cmap="RdBu_r", center=0, annot=False)
        plt.title(f"Interaction Effect (Delta) Heatmap - Variant {variant}")
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"interaction_heatmap_{variant}.png"), bbox_inches="tight")
        plt.savefig(os.path.join(output_dir, f"interaction_heatmap_{variant}.pdf"), bbox_inches="tight")
        plt.close()
...
def main():
    ...
    results_df = results_df.sort_values(["significant", "abs_delta", "delta"], ascending=[False, False, True]).reset_index(drop=True)
    ...
    plt.figure(figsize=(12, 10))
    sns.heatmap(pivot_delta, cmap="RdBu_r", center=0, annot=False)
    plt.title("Interaction Effect (Delta) Heatmap (Raw Variant-Aware Bootstrap DiD)")
    plt.tight_layout()
    ...
    plot_variant_heatmaps(raw_df, OUTPUT_DIR)
