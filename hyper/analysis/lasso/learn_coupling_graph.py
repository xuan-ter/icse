...
DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "results", "20260531_191832"))
...
RAW_RESULTS_CSV = os.path.join(DATA_DIR, "experiment_results.csv")
...
def load_and_preprocess_data():
    """
    Loads raw repeated experiment results and converts them into a feature matrix X
    and target vector y suitable for Lasso regression.

    X: Binary matrix where 1 means the pass is disabled.
    y: Log-transformed per-run runtime.
    """
    print("Loading data...")
    ...
    if "Status" in full_df.columns:
        full_df = full_df[full_df["Status"].astype(str).str.strip().eq("Success")].copy()

    # Ensure runtime is positive
    full_df = full_df[pd.to_numeric(full_df["TotalRuntime(s)"], errors="coerce") > 0].copy()
    ...
    print(f"Raw run samples used: {len(y)}")
    return X_full, y, feature_names, mir_passes, llvm_passes

def run_lasso_stability_selection(X, y, feature_names, n_bootstrap=100):
    """
    Runs Lasso on raw repeated-run data.
    Uses LassoCV on the full dataset to choose alpha, then bootstraps rows to
    estimate selection stability and average coefficients.
    """
    print("Running Lasso on raw repeated-run data...")
    ...
    coeff_samples = []
    nonzero_counts = np.zeros(X.shape[1], dtype=float)
    ...
    for b in range(max(1, int(n_bootstrap))):
        ...
        coeffs = lasso.coef_
        coeff_samples.append(coeffs)
        nonzero_counts += (np.abs(coeffs) > 1e-5).astype(float)

        if (b + 1) % 10 == 0 or b == 0 or b + 1 == n_bootstrap:
            print(f"  Bootstrap fits completed: {b + 1}/{n_bootstrap}")

    coeff_matrix = np.vstack(coeff_samples)
    stability_scores = nonzero_counts / max(1, int(n_bootstrap))
    avg_coeffs = coeff_matrix.mean(axis=0)
...
def save_coupling_graph(stability_scores, avg_coeffs, feature_names, threshold=0.0):
    ...
    for i, (score, coeff, name) in enumerate(zip(stability_scores, avg_coeffs, feature_names)):
        ...
        if '|' in name and score > threshold and abs(coeff) > 1e-5:
            interaction_coeffs.append((i, score, abs(coeff), coeff, name))

    # Sort by coefficient magnitude and take top ones
    interaction_coeffs.sort(key=lambda x: (x[1], x[2]), reverse=True)
    top_n = min(100, len(interaction_coeffs))  # Take top 100 or all

    for i, score, abs_coeff, coeff, name in interaction_coeffs[:top_n]:
        parts = name.split('|')
        mir = parts[0]
        llvm = parts[1]
...
def plot_coupling_graph(df_edges, mir_passes, llvm_passes):
    ...
    plt.title("Cross-Level Coupling Graph (Lasso on Raw Runs)\nSquare=MIR, Circle=LLVM | Red=Conflict(+), Blue=Synergy(-)")
    plt.axis('off')
