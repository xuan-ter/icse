...
DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "results", "run_20260619_210845"))
OUTPUT_DIR = BASE_DIR
...
def normalize_pass_names(df):
    """
    Normalize pass names and map the recorded all-off baseline to (None, None).
    """
    def clean_name(name):
        if pd.isna(name):
            return "None"
    ...
    # rustls records the all-off baseline as (MIR=All, LLVM=None).
    # Remap it to the analysis baseline (None, None) so y00 exists.
    df.loc[(df["MIR_Pass"] == "All") & (df["LLVM_Pass"] == "None"), "MIR_Pass"] = "None"
