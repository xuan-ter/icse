...
PROJECT_ROOT = "/root/MIR_LLVM/bat"
DEFAULT_JSON_ROOT = "/root/MIR_LLVM/table"
DEFAULT_RESULTS_ROOT = "/root/MIR_LLVM/bat/results"
DEFAULT_BENCH_FILE = os.path.join(PROJECT_ROOT, "benchmark_data.rs")
BENCH_VARIANT = "bat"


def ensure_bench_file(bench_path=DEFAULT_BENCH_FILE):
    desired_bytes = 10 * 1024 * 1024
    if os.path.exists(bench_path):
        try:
            size = os.path.getsize(bench_path)
        except Exception:
            size = 0
        if desired_bytes <= size < desired_bytes * 1.5:
            return bench_path
        try:
...
def resolve_json_path(path_value):
    candidate = path_value.strip() if path_value else ""
    if not candidate:
        candidate = DEFAULT_JSON_ROOT
    ...
    if os.path.isdir(candidate):
        ...
        for option in options:
            if os.path.exists(option):
                return option
        raise SystemExit(f"JSON file not found under directory: {candidate}")
...
def get_exe_path():
    exe_name = "bat.exe" if os.name == "nt" else "bat"
    exe_path = os.path.join(PROJECT_ROOT, "target", "release", exe_name)
    return exe_path if os.path.exists(exe_path) else None


def run_benchmark(exe_path, bench_file, repeats, warmup, extra_args):
    args = [exe_path, "--style=full", "--color=always", "--paging=never", bench_file]
    if extra_args:
        args = [exe_path] + extra_args + [bench_file]
...
def aggregate_summary(rows):
    ...
    for (variant, cfg), rs in sorted(by.items()):
        ...
        runtime_mean = avg_value("TotalRuntime(s)")
        runtime_med = med_value("TotalRuntime(s)")
        runtime_iqr = iqr_value("TotalRuntime(s)")
        compile_mean = avg_value("CompileTime(s)")
        compile_med = med_value("CompileTime(s)")
        compile_iqr = iqr_value("CompileTime(s)")
        size_mean = avg_value("BinarySize(Bytes)", int)
        size_med = med_value("BinarySize(Bytes)", int)
        ...
        out.append(
            {
                ...
                "ConfigName": cfg,
                "n": str(len(rs)),
                "runtime_mean": f"{runtime_mean:.6f}" if runtime_mean is not None else "",
                "runtime_med": f"{runtime_med:.6f}" if runtime_med is not None else "",
                "runtime_iqr": f"{runtime_iqr:.6f}" if runtime_iqr is not None else "",
                "compile_mean": f"{compile_mean:.6f}" if compile_mean is not None else "",
                "compile_med": f"{compile_med:.6f}" if compile_med is not None else "",
                "compile_iqr": f"{compile_iqr:.6f}" if compile_iqr is not None else "",
                "size_mean": f"{size_mean:.6f}" if size_mean is not None else "",
                "size_med": f"{size_med:.0f}" if size_med is not None else "",
            }
        )
    return out
...
def measure_combination(combo, runs, skip_clean, bench_file, run_repeats, warmup, extra_args, logf):
    ...
    msg3 = f"[Run] runs={runs}, repeats={run_repeats}, warmup={warmup}, bench={bench_file}"
    print(msg1)
    print(msg2)
    print(msg3)
    logf.write(msg1 + "\n")
    ...
    for run_id in range(1, runs + 1):
        ...
        total_runtime = run_benchmark(exe_path, bench_file, run_repeats, warmup, extra_args)
        ...
        print(f"[Result] Size={size}B, Compile={compile_time:.6f}s, RunTotal={total_runtime:.6f}s")
        results.append(
            {
                "ConfigName": name,
                "Variant": BENCH_VARIANT,
                ...
                "MIR_Pass": mir_pass_label,
                "BinarySize(Bytes)": size,
                "TotalRuntime(s)": f"{total_runtime:.6f}",
                "CompileTime(s)": f"{compile_time:.6f}",
                "RunRepeats": run_repeats,
                "Warmup": warmup,
                "BenchFile": bench_file,
...
def main():
    ...
    parser.add_argument("--bench-file", default="")
    parser.add_argument("--run-repeats", type=int, default=1)
    parser.add_argument("--warmup", type=int, default=1)
    parser.add_argument("--bat-args", default="")
    args = parser.parse_args()

    extra_args = []
    if args.bat_args.strip():
        extra_args = args.bat_args.strip().split()

    config_path = resolve_json_path(args.json_path.strip() or str(args.config_file))

    if args.bench_file.strip():
        bench_file = os.path.abspath(args.bench_file)
        if not os.path.exists(bench_file):
            raise SystemExit(f"Bench file not found: {bench_file}")
    else:
        bench_file = ensure_bench_file()
