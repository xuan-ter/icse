...
DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "results", "run_20260622_162101"))
OUTPUT_DIR = BASE_DIR
...
def normalize_pass_names(df):
    """
    Normalize baseline spellings and remove synthetic aggregate rows.
    """
    def clean_name(name):
        if pd.isna(name):
            return "None"
        s = str(name).strip()
        if s.lower() in ["none", "baseline", "nan", "", "n/a", "na"]:
            return "None"
        return s
    ...
    # image stores the DiD baseline as EXP_DBL_000_BASELINE with
    # (LLVM_Pass=baseline, MIR_Pass=baseline), which is normalized to (None, None).
    # Drop remaining synthetic aggregate rows like (None, All) or (All, None).
    df = df[(df["MIR_Pass"] != "All") & (df["LLVM_Pass"] != "All")].copy()
    return df


def load_raw_experiment_data():
    """
    Load repeated raw runs and keep only positive-runtime samples.
    """
...
def analyze_interactions(raw_df, metric="TotalRuntime(s)", n_bootstrap=BOOTSTRAP_SAMPLES, seed=BOOTSTRAP_SEED):
    """
    Run full DiD analysis directly on repeated raw runs, with bootstrap wrapping
    the DiD estimator for every MIR/LLVM pair.
    """
