...
PROJECT_ROOT = "/root/MIR_LLVM/ripgrep"
BINARY_PATH = os.path.join(PROJECT_ROOT, "target", "release", "rg")
DEFAULT_JSON_ROOT = "/root/MIR_LLVM/table"
DEFAULT_JSON_PATH = os.path.join(DEFAULT_JSON_ROOT, "table_json", "combined_experiment_matrix.json")
DEFAULT_RESULTS_ROOT = "/root/MIR_LLVM/ripgrep/results"
BENCH_VARIANT = "rg"
...
def build_project(env, rustflags, logf, retries=2):
    ...
    for _ in range(retries + 1):
        r = subprocess.run(
            ["cargo", "+nightly", "build", "--release", "--package", "ripgrep", "--bin", "rg", "--quiet"],
            cwd=PROJECT_ROOT,
            env=env2,
...
def newest_llvm_ir_path():
    patterns = [
        os.path.join(PROJECT_ROOT, "target", "release", "deps", "rg-*.ll"),
        os.path.join(PROJECT_ROOT, "target", "release", "deps", "ripgrep-*.ll"),
    ]
    candidates = []
...
def run_benchmark_once(exe_path, bench_file):
    t0 = time.perf_counter()
    p = subprocess.run(
        [exe_path, "-c", "Sherlock", bench_file],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.PIPE,
        text=True,
    )
...
def run_benchmark_repeated(exe_path, bench_file, repeats):
    ...
    for _ in range(repeats):
        elapsed, err = run_benchmark_once(exe_path, bench_file)
        if elapsed is None:
            return None, err
        total += elapsed
    return total, ""
...
def calibrate_repeats(env_base, baseline_combo, bench_file, target_seconds, max_repeats, skip_clean, logf):
    ...
    elapsed, _ = run_benchmark_once(exe_path, bench_file)
        ...
        with open(results_csv, "w", encoding="utf-8", newline="") as outcsv:
            ...
            for combo in combos:
                ...
                for warmup_id in range(1, warmup_count + 1):
                    ...
                    logf.flush()
                    wall_time, err = run_benchmark_repeated(exe_path, bench_file, run_repeats)
